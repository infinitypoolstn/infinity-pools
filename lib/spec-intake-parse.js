...
async function parseIntake(buffer) {
  ...
  const specs = {
    ...
    coldPlunge: { included: ck('cp_inc') || !!(t('cp_det') || t('cp_led')), price: 0, details: t('cp_det'), ledLights: t('cp_led') },
  ...
  return { name: t('owner_builder'), address: t('address'), phone: t('phone'), email: '', notes, specs };
}
