...
const SCOPE_TEMPLATE = [
  { key: 'design', title: 'Design', items: [
    ...
    'Standard Pebble Tec & Pebble Sheen colors included. Other finish colors are available, but will require additional cost. Color chart attached in proposal.',
...
function newClient({ name, address, email, phone }) {
  return {
    ...
    specs: {
      shape: 'geometric', sizeDetails: '',
      hotTub: { included: false, details: '', jets: '', ledLights: '' },
      sunShelf: { included: false, details: '' },
      spillover: { included: false, details: '' },
      ledgeSeating: { included: false, details: '', style: 'Baja Ledge' },
      waterFeature: { included: false, details: '' },
      fireFeature: { included: false, details: '' },
      coldPlunge: { included: false, details: '' },
      jets: '', ledLights: '', equipmentPad: '',
      addOns: [],
    },
    specsLocked: false,
...
function load() {
  ...
  if (fs.existsSync(DATA_FILE)) {
    ...
    // back-fill spec fields added after a client was created
    for (const c of (data.clients || [])) {
      if (c.specs && c.specs.fireFeature === undefined) c.specs.fireFeature = { included: false, details: '' };
      if (c.specs && c.specs.coldPlunge === undefined) c.specs.coldPlunge = { included: false, details: '' };
      if (c.specs && c.specs.hotTub) {
        if (c.specs.hotTub.jets === undefined) c.specs.hotTub.jets = '';
        if (c.specs.hotTub.ledLights === undefined) c.specs.hotTub.ledLights = '';
      }
    }
  } else {
    data = defaults();
