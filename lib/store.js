...
// Default Project Overview paragraph shown on the contract below the pool
// sections. Copied onto each new client so it can be edited per project.
...
const FINISHES_SEED = [
  ...
  { brand: 'PebbleFina', tier: 'Premium', name: 'Egyptian Sands', shimmer: true, imageUrl: U + '2021/11/pf-sample-eqyptian-sands-1200x1000.jpg', color: '#d8c39c' },
...
// Master task lists for the optional Site Excavation and Landscaping modules
// (keyed by module key). When a module's stage is started these tasks are copied
// onto the project, just like the phase task workflows above. Editable in Settings.
const MODULE_TASK_TEMPLATES = {
  siteExcavation: [
    { title: 'Confirm site access route & staging area with GC', dueOffsetDays: 1 },
    { title: 'Schedule excavation crew & equipment', dueOffsetDays: 2 },
    { title: 'Arrange haul-off / dirt disposal', dueOffsetDays: 2 },
    { title: 'Rough grade site to plan', dueOffsetDays: 5 },
  ],
  landscaping: [
    { title: 'Finalize landscape plan & plant selections with client', dueOffsetDays: 2 },
    { title: 'Order plants, sod, and materials', dueOffsetDays: 4 },
    { title: 'Schedule irrigation & planting crew', dueOffsetDays: 5 },
    { title: 'Final walkthrough & cleanup', dueOffsetDays: 10 },
  ],
};
...
const FILE_CATEGORIES = ['Plans', 'Pool Renderings', 'Landscape Plans', 'Permits', 'Material Invoices', 'Labor Invoices', 'Other'];
...
function defaults() {
  return {
    settings: {
      ...
      // Dirtworks Ledger integration — pull a linked job's expenses into a
      // project's Site Excavation costs. exportToken must match EXPORT_TOKEN on
      // the Dirtworks service.
      dirtworks: {
        url: 'https://dirtworks-ledger.onrender.com',
        exportToken: '',
      },
      haulRates: {
        triAxle: 500,
      ...
      moduleTaskTemplates: JSON.parse(JSON.stringify(MODULE_TASK_TEMPLATES)),
    },
    employees: [],
    contractors: [],
    clients: [],
    tasks: [],
    // Scheduled project events shown on the Calendar. Each: { id, clientId, title,
    // details, date 'YYYY-MM-DD', time 'HH:MM'|'', employeeId, remindersSent, createdAt }.
    events: [],
    alerts: [],
    outbox: [],
    errorLog: [],
...
function freshPhases() {
  ...
  return src.map(p => ({
    ...
    // Progress-invoicing refs — set when this draw's QuickBooks invoice is created.
    qbInvoiceId: null, qbInvoiceUrl: null,
  }));
}

// Repair project details (projectType === 'repair'): what's needed (items with
// costs), a deadline, a budget, who pays, and the QuickBooks invoice sent to the
// client when they're responsible. items: [{ id, description, cost }].
function freshRepair() {
  return {
    items: [],
    deadline: null,           // YYYY-MM-DD
    budget: 0,
    responsible: 'client',    // 'client' | 'infinity'
    paymentLink: '',          // manual pay link when QuickBooks isn't connected
    qb: { invoiceId: null, invoiceUrl: null, payLink: null, sentAt: null },
  };
}

// Optional project module (Site Excavation / Landscaping) on a New Pool project.
// A lightweight, separately-priced stage: an include toggle, free-text details, a
// list of priced line items ({ label, value, price }), and a simple status the team
// advances (pending → active → complete) so the client can see the stage. Prices are
// kept as their own subtotal (moduleTotal) — NOT folded into the pool quote/contract.
function freshModule() {
  return { included: false, details: '', items: [], status: 'pending', startedAt: null, completedAt: null };
}

// Subtotal of a module's priced line items.
function moduleTotal(m) { return sum((m && m.items || []).map(i => Number(i.price) || 0)); }

// Default team checklist seeded onto every project. Admin-editable per project
// (add/rename/remove); shown read-only on the Employee View.
function freshChecklist() {
  return [
    { id: id(), label: 'Submit Permit', done: false, completedAt: null },
    { id: id(), label: 'Permit Approved', done: false, completedAt: null },
    { id: id(), label: 'MEPs Complete', done: false, completedAt: null },
  ];
}
...
function freshSpecs() {
  return {
    ...
    // Pool Base, Spa Base, Fire Lounge, and Cold Plunge each carry an `items` list of extra
    // priced line items (each { label, value, price }) that roll into the quote.
    spaBase: { included: false, price: 0, size: '', jets: '', ledLights: '', details: '', items: [] },
    fireLounge: { included: false, price: 0, size: '', details: '', items: [] },
    waterFeature: { included: false, price: 0, details: '' },
    coldPlunge: { included: false, price: 0, details: '', ledLights: '', additionalDetails: '', items: [] },
...
function migrateSpecs(c) {
  ...
  if (!s.poolBase) {
    c.specs = {
      ...
      equipmentPad: s.equipmentPad || '',
      addOns: (s.addOns || []).map(a => ({ label: a.label || '', value: a.value || '', price: Number(a.price) || 0 })),
    };
  }
  if (c.specs && c.specs.equipmentPad === undefined) c.specs.equipmentPad = '';
  ...
  if (pb) {
    ...
    delete pb.shapeText;
    // Legacy/manual entry sometimes crammed the depth into the Size field
    // ("15' x 25', 3.5-6' depth") with Depth left blank — split it back out so
    // Size and Depth show on their own lines everywhere. Only acts when the last
    // comma/semicolon-separated chunk mentions depth, so real sizes are untouched.
    if (!(pb.depth || '').trim() && /[,;]/.test(pb.size || '')) {
      const parts = pb.size.split(/[,;]/);
      const last = parts.length >= 2 ? parts[parts.length - 1].trim() : '';
      if (/\d/.test(last) && /(depth|deep)\.?$/i.test(last)) {
        parts.pop();
        pb.size = parts.join(',').trim();
        pb.depth = last.replace(/\s*(depth|deep)\.?$/i, '').replace(/[\s,;]+$/, '').trim();
      }
    }
  }
  if (c.specs.spaBase && c.specs.spaBase.details === undefined) c.specs.spaBase.details = '';
  if (c.specs.coldPlunge && c.specs.coldPlunge.ledLights === undefined) c.specs.coldPlunge.ledLights = '';
  if (c.specs.coldPlunge && c.specs.coldPlunge.additionalDetails === undefined) c.specs.coldPlunge.additionalDetails = '';
  // Fire Lounge was added after Spa Base; back-fill it and the per-section priced
  // `items` lists so older saved specs read cleanly everywhere.
  if (!c.specs.fireLounge) c.specs.fireLounge = { included: false, price: 0, size: '', details: '', items: [] };
  for (const k of ['poolBase', 'spaBase', 'fireLounge', 'coldPlunge']) {
    if (c.specs[k] && !Array.isArray(c.specs[k].items)) c.specs[k].items = [];
  }
}
...
function specsToFinance(specs) {
  ...
  const items = [];
  // A priced section plus each of its extra line items (labelled "Section — Item"
  // so estimate/contract detail lookups still line up).
  const pushSection = (obj, title) => {
    if (!(obj && obj.included)) return;
    items.push({ id: id(), label: title, amount: num(obj.price) });
    for (const it of (obj.items || [])) {
      if ((it.label || '').trim()) items.push({ id: id(), label: title + ' — ' + it.label, amount: num(it.price) });
    }
  };
  items.push({ id: id(), label: 'Pool Base', amount: num(s.poolBase && s.poolBase.price) });
  for (const it of ((s.poolBase && s.poolBase.items) || [])) {
    if ((it.label || '').trim()) items.push({ id: id(), label: 'Pool Base — ' + it.label, amount: num(it.price) });
  }
  pushSection(s.spaBase, 'Spa Base');
  pushSection(s.fireLounge, 'Fire Lounge');
  if (s.waterFeature && s.waterFeature.included) items.push({ id: id(), label: 'Water Feature', amount: num(s.waterFeature.price) });
  pushSection(s.coldPlunge, 'Cold Plunge');
...
function resetBuild(client) {
  ...
  // Reset module stage progress but keep their pricing/details/included choice.
  for (const k of ['siteExcavation', 'landscaping']) {
    if (client[k]) Object.assign(client[k], { status: 'pending', startedAt: null, completedAt: null });
  }
  client.changeOrders = [];
  client.contract = { sentAt: null, signedAt: null, signedMethod: null, depositMethod: null };
  client.quickbooks = { invoiceId: null, invoiceUrl: null, estimateId: null, estimateUrl: null, payLink: null, qbCustomerId: null, qbCustomerName: '' };
  // Start Over wipes the build workflow, so drop this project's tasks and
  // scheduled calendar events too.
  data.tasks = (data.tasks || []).filter(t => t.clientId !== client.id);
  data.events = (data.events || []).filter(e => e.clientId !== client.id);
  save();
}

function newClient({ name, address, email, phone, testMode = false, projectType = 'new_pool' }) {
  return {
    id: id(),
    portalToken: token(),
    // A separate token for a read-only, no-login shareable portal link (view all
    // details, but cannot sign or make binding decisions). Regenerate to revoke.
    viewToken: token(),
    createdAt: new Date().toISOString(),
    name: name || '', address: address || '', email: email || '', phone: phone || '',
    // Extra people (spouse, builder, etc.) who receive the portal link and can
    // log into the portal with their own email. The primary `email` is always included.
    additionalEmails: [],
    // Optional second authorized contract signer. Together with the primary
    // `email` these are the two people allowed to sign / take binding portal
    // actions — either one can sign the contract.
    secondSignerEmail: '',
    // Billing-only recipients (an office manager, bookkeeper, spouse who pays the
    // bills). They are copied on invoices and phase payment requests but get NO
    // portal access and cannot sign — unlike additionalEmails.
    billingEmails: [],
    // A test job lets you walk the full workflow without ever creating a real
    // QuickBooks invoice. Everything else behaves normally.
    testMode: !!testMode,
    // 'new_pool' runs the full build workflow; 'repair' uses the Repair tab (scope
    // of work, deadline, budget, who pays, and — if the client pays — an invoice).
    projectType: projectType === 'repair' ? 'repair' : 'new_pool',
    repair: freshRepair(),
    // Optional separately-priced stages on a New Pool project, in timeline order:
    // Site Excavation (before the pool) and Landscaping (after). See freshModule().
    siteExcavation: freshModule(),
    landscaping: freshModule(),
    // Free-text project contacts shown on the Overview tab (name / phone / email
    // per role). Team-only — never client-facing.
    contacts: { builder: '', contractor: '', other: '' },
    status: 'prospect', // prospect | contract_sent | active | completed | lost
    targetFinishDate: null, // YYYY-MM-DD — projected completion, shown on the dashboard
    specs: freshSpecs(),
    specsLocked: false,
    scope: JSON.parse(JSON.stringify((data && data.settings && data.settings.scopeTemplate) || SCOPE_TEMPLATE)),
    // Per-project copy of the disclosures, seeded from the master (Settings) and
    // editable on the project's Disclosures tab. The contract PDF uses this copy.
    disclosures: JSON.parse(JSON.stringify((data && data.settings && data.settings.disclosures) || DISCLOSURES_TEMPLATE.map(d => ({ id: id(), ...d })))),
    ...
    selectedFinishes: [],
    clientTodos: [],
    // Team checklist (admin-editable, shown read-only on the Employee View).
    checklist: freshChecklist(),
    hauls: { triAxle: 0, gravel: 0 },
    contract: { sentAt: null, signedAt: null, signedMethod: null, depositMethod: null },
    // Portal open tracking — recorded when the client views their portal (see
    // recordPortalVisit in server.js). log: [{ at, ip }], newest first.
    portalAccess: { firstAt: null, lastAt: null, count: 0, log: [] },
    quickbooks: { invoiceId: null, invoiceUrl: null, estimateId: null, estimateUrl: null, estimateNumber: null, estimateClosedAt: null, payLink: null, qbCustomerId: null, qbCustomerName: '' },
    notes: '',
    // Dated Pool Specs notes, newest first: [{ id, at, text }].
    specNotes: [],
    // Post-signing build-detail change log: [{ at, changes:[{label, from, to}] }].
    // specsSignedSummary snapshots the spec summary at signing (the "original").
    specHistory: [],
    specsSignedSummary: null,
  };
}
...
function load() {
  ...
  if (fs.existsSync(DATA_FILE)) {
    ...
    // Adobe Sign was replaced by DocuSeal — drop its stale settings/credentials.
    delete data.settings.adobeSign;
    // Back-fill the Landscape Plans upload category on installs saved before it existed.
    if (Array.isArray(data.settings.fileCategories) && !data.settings.fileCategories.includes('Landscape Plans')) {
      const arr = data.settings.fileCategories, i = arr.indexOf('Other');
      arr.splice(i >= 0 ? i : arr.length, 0, 'Landscape Plans');
    }
    // migrate pre-restructure specs onto the five-section priced shape
    for (const c of (data.clients || [])) {
      migrateSpecs(c);
      if (c.projectOverview === undefined) c.projectOverview = DEFAULT_PROJECT_OVERVIEW;
      // Back-fill the projected completion for already-signed pools: 3 months after
      // signing. Stays editable; only fills when unset.
      if (c.targetFinishDate === undefined) c.targetFinishDate = null;
      if (c.projectType === undefined) c.projectType = 'new_pool';
      if (!c.repair) c.repair = freshRepair();
      if (!c.siteExcavation) c.siteExcavation = freshModule();
      if (!c.landscaping) c.landscaping = freshModule();
      if (!Array.isArray(c.additionalEmails)) c.additionalEmails = [];
      if (typeof c.secondSignerEmail !== 'string') c.secondSignerEmail = '';
      if (!Array.isArray(c.billingEmails)) c.billingEmails = [];
      if (c.quickbooks && c.quickbooks.qbCustomerId === undefined) { c.quickbooks.qbCustomerId = null; c.quickbooks.qbCustomerName = ''; }
      if (c.quickbooks && c.quickbooks.estimateNumber === undefined) { c.quickbooks.estimateNumber = null; c.quickbooks.estimateClosedAt = null; }
      if (!Array.isArray(c.disclosures)) c.disclosures = JSON.parse(JSON.stringify(data.settings.disclosures || []));
      if (!c.viewToken) c.viewToken = token();
      if (!c.contacts) c.contacts = { builder: '', contractor: '', other: '' };
      if (!Array.isArray(c.checklist)) c.checklist = freshChecklist();
      // One-time: add "Submit Permit" to existing projects' checklists (front),
      // unless already present. Guarded so a later manual delete isn't undone.
      if (!data._checklistSubmitPermit && Array.isArray(c.checklist)
          && !c.checklist.some(it => (it.label || '').trim().toLowerCase() === 'submit permit')) {
        c.checklist.unshift({ id: id(), label: 'Submit Permit', done: false, completedAt: null });
      }
      if (!Array.isArray(c.specNotes)) c.specNotes = [];
      if (!Array.isArray(c.specHistory)) c.specHistory = [];
      if (!c.portalAccess) c.portalAccess = { firstAt: null, lastAt: null, count: 0, log: [] };
      if (!c.targetFinishDate && c.contract && c.contract.signedAt) {
        const t = new Date(c.contract.signedAt); t.setMonth(t.getMonth() + 3);
        c.targetFinishDate = t.toISOString().slice(0, 10);
      }
    }
    data._checklistSubmitPermit = true; // one-time "Submit Permit" backfill done
    // One-time: add/correct finishes newly published by Pebble Tec that admins
    // asked us to include, with their canonical tier + swatch. Bump the version
    // flag to re-apply after changing the values below; a later manual edit is
    // respected once the flag is set.
    if (!data._newFinishesV2) {
      const NEW_FINISHES = [
        { brand: 'PebbleFina', tier: 'Premium', name: 'Egyptian Sands', shimmer: true, imageUrl: U + '2021/11/pf-sample-eqyptian-sands-1200x1000.jpg', color: '#d8c39c' },
      ];
      for (const nf of NEW_FINISHES) {
        const existing = (data.finishes || []).find(f => f.brand === nf.brand && f.name.toLowerCase() === nf.name.toLowerCase());
        if (existing) {
          existing.tier = nf.tier; existing.shimmer = !!nf.shimmer;
          if (nf.imageUrl && !existing.imageUrl) existing.imageUrl = nf.imageUrl; // don't clobber a locally-cached swatch
        } else {
          data.finishes.push({ id: id(), active: true, source: 'manual', shimmer: !!nf.shimmer, ...nf });
        }
      }
      data._newFinishesV2 = true;
    }
  } else {
    data = defaults();
    saveNow();
...
// Only approved change orders count toward the contract total. Legacy COs (saved
// before the approval flow) have no status and are treated as approved.
function changeOrderTotal(client) { return sum((client.changeOrders || []).filter(c => (c.status || 'approved') === 'approved').map(c => c.value)); }
...
/**
 * Invoice-only recipients on a job: copied on QuickBooks invoices and phase
 * payment requests, never given portal access (that's `additionalEmails`).
 * Normalized — trimmed, de-duplicated, non-empty.
 */
function billingEmails(c) {
  return [...new Set(((c && c.billingEmails) || []).map(e => String(e || '').trim()).filter(Boolean))];
}

module.exports = {
  load, save, saveNow, id, token, newClient, resetBuild, addAlert, addError,
  freshSpecs, migrateSpecs, specsToFinance,
  quoteTotal, changeOrderTotal, contractTotal, costTotal, phaseAmount, collected, currentPhase, moduleTotal,
  billingEmails,
  get data() { return data; },
  PHASE_TEMPLATE, SCOPE_TEMPLATE, FINISHES_SEED, DEFAULT_PROJECT_OVERVIEW,
};
