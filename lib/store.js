...
const FINISHES_SEED = [
  ...
  { brand: 'PebbleFina', tier: 'Upgrade', name: 'Egyptian Sands', imageUrl: null, color: '#d8c39c' },
...
function load() {
  ...
  if (fs.existsSync(DATA_FILE)) {
    ...
    // One-time: add finishes newly published by Pebble Tec that admins asked us to
    // include. Guarded by a flag so a later delete/retire is respected.
    if (!data._seededNewFinishes) {
      const NEW_FINISHES = [
        { brand: 'PebbleFina', tier: 'Upgrade', name: 'Egyptian Sands', imageUrl: null, color: '#d8c39c' },
      ];
      for (const nf of NEW_FINISHES) {
        const has = (data.finishes || []).some(f => f.brand === nf.brand && f.name.toLowerCase() === nf.name.toLowerCase());
        if (!has) data.finishes.push({ id: id(), active: true, source: 'manual', shimmer: !!nf.shimmer, ...nf });
      }
      data._seededNewFinishes = true;
    }
  } else {
    data = defaults();
    saveNow();
