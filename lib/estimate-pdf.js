...
async function generate(client) {
  ...
  items.forEach((it, i) => {
    ...
    // Detail lines are strings, or { text, bold } for emphasized labels.
    const lineText = l => typeof l === 'string' ? l : (l && l.text) || '';
    const lineBold = l => typeof l === 'object' && l && !!l.bold;
    // Measure the detail block so a row's title and its details never split
    // across a page break.
    doc.font(BODY).fontSize(9.5);
    let detailH = 0;
    for (const l of lines) detailH += doc.heightOfString('– ' + lineText(l), { width: detailW, lineGap: 1 }) + 2;
    ...
    for (const l of lines) {
      doc.font(lineBold(l) ? BOLD : BODY);
      const txt = '– ' + lineText(l);
      const hh = doc.heightOfString(txt, { width: detailW, lineGap: 1 });
      doc.text(txt, detailX, dy, { width: detailW, lineGap: 1 });
      dy += hh + 2;
    }
    ty += blockH;
  });
