...
async function generate(client) {
  ...
  // (that's payment-schedule info, not relevant to plans/permitting). Phases that
  // aren't relevant to plan drafting / permitting are omitted from this sheet.
  const PLANS_EXCLUDE_KEYS = new Set(['design', 'lotprep', 'excavation', 'completion', 'landscaping']);
  const PLANS_EXCLUDE_TITLES = new Set(['design', 'lot preparation', 'excavation', 'completion & activation', 'landscaping']);
  const scope = (client.scope || []).filter(sec =>
    !PLANS_EXCLUDE_KEYS.has(sec.key) &&
    !PLANS_EXCLUDE_TITLES.has(String(sec.title || '').trim().toLowerCase()));
  if (scope.length) {
  ...
  // allDetails: the drafter/permit office needs every note, including the Spa's
  // free-text additional details that the contract's spec breakdown trims.
  const specSecs = buildSpecSections(client, { allDetails: true });
  if (specSecs.length) {
