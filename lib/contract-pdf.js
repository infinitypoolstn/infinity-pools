...
function buildSpecSections(client) {
  ...
  // Spa Base keeps Size / Jets / LED but not its free-text "Size and Details".
  // Cold Plunge and Fire Feature show the section header + price only. Water
  // Feature keeps its detail line.
  const spa = s.spaBase || {};
  if (spa.included) {
    const lines = [];
