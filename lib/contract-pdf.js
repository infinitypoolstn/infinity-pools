...
function buildSpecSections(client) {
  ...
  // Spa Base and Fire Lounge keep Size / Jets / LED but not their free-text
  // additional details. Fire Lounge mirrors Spa Base and follows it.
  const sizeSection = (obj, title) => {
    if (!(obj && obj.included)) return;
    const lines = [];
    if (obj.size) lines.push('Size: ' + obj.size);
    if (obj.jets) lines.push('Number of Jets: ' + obj.jets);
    if (obj.ledLights) lines.push('Hayward Colorlogic 320 LED Lights: ' + obj.ledLights);
    sections.push({ title, price: Number(obj.price) || 0, lines });
    pushSubItems(obj, title);
  };
  sizeSection(s.spaBase, 'Spa Base');
  sizeSection(s.fireLounge, 'Fire Lounge');
