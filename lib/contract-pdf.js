...
function header(doc, title) {
  ...
  doc.fillColor('#ffffff').font('Helvetica-Bold').fontSize(20).text('INFINITY POOLS', 50, 22, { characterSpacing: 3 });
  doc.font('Helvetica').fontSize(11).text(title.toUpperCase(), 50, 48, { characterSpacing: 2 });
  doc.fillColor(DARK);
  doc.y = 110;
}
...
async function generate(client, { uploadsDir }) {
  ...
  if (cover) {
    ...
    if (fs.existsSync(imgPath)) {
      ...
      if (buf) doc.image(buf, 40, 120, { fit: [doc.page.width - 80, 380], align: 'center', valign: 'center' });
    }
  }
  doc.fillColor('#ffffff');
  doc.font('Helvetica-Bold').fontSize(34).text(client.address.toUpperCase(), 50, 56, { width: doc.page.width - 100 });
  doc.font('Helvetica').fontSize(13).text('Pool Construction Proposal & Contract', 50, doc.y + 6);
  const footY = doc.page.height - 150;
  doc.font('Helvetica-Bold').fontSize(22).text('INFINITY POOLS', 50, footY, { characterSpacing: 4 });
  doc.font('Helvetica').fontSize(11)
    .text('Prepared for ' + client.name, 50, footY + 34)
    .text('Prepared by ' + (store.data.settings.companyName || 'Infinity Pools'), 50, footY + 50)
    .text(today, 50, footY + 66);
  ...
  if (renderings.length > 0) {
    ...
    for (let i = 0; i < renderings.length; i++) {
      ...
      doc.font(f.isCoverPhoto ? 'Helvetica-Bold' : 'Helvetica').fontSize(8).fillColor(f.isCoverPhoto ? BLUE : MID)
  ...
  y += 40;
  doc.fillColor(MID).font('Helvetica-Oblique').fontSize(9)
  ...
  for (const brand of brandOrder) {
    ...
    doc.moveDown(0.5);
    doc.font('Helvetica-Bold').fontSize(12).fillColor(BLUE).text(brand, 50);
    for (const tier of brandTiers) {
      const fin = groups[brand + '|' + tier];
      if (doc.y > doc.page.height - 120) doc.addPage();
      doc.font('Helvetica-Bold').fontSize(9).fillColor(MID).text(tier.toUpperCase(), 50, doc.y + 4);
      ...
      for (const f of fin) {
        ...
        doc.fillColor(DARK).font(isSel ? 'Helvetica-Bold' : 'Helvetica')
  ...
  doc.moveDown(0.6);
  doc.font('Helvetica-Oblique').fontSize(8).fillColor(MID)
    .text('* Denotes the finish contains Shimmering Sea™ seashell blend in the mix.', 50);
  if (selected.size) {
    doc.moveDown(0.5);
    doc.font('Helvetica-Bold').fontSize(10).fillColor(BLUE).text('Client selections: ' + [...selected].join(', '), 50);
  }
  ...
  store.data.settings.disclosures.forEach((d, i) => {
    ...
    for (const para of d.body.split('\n')) {
      ...
      if (doc.y > doc.page.height - 90) doc.addPage();
      doc.font('Helvetica').fontSize(9).fillColor(DARK).text(para, 56, doc.y, { width: doc.page.width - 112, lineGap: 2 });
      doc.moveDown(0.2);
    }
  });
  ...
  doc.moveDown(1.5);
  doc.font('Helvetica-Bold').fontSize(11).fillColor(BLUE).text('Contract total: ' + money(total));
  doc.moveDown(1.2);
  ...
  let yy = lineY();
  doc.font('Helvetica').fontSize(10).fillColor(DARK).text('Client Signature', 50, yy + 4);
  ...
  for (let i = 0; i < range.count; i++) {
    ...
    doc.font('Helvetica').fontSize(8).fillColor(i === 0 ? '#ffffff' : MID)
