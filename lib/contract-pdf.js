...
function buildSpecSections(client) {
  ...
  if (pb.shapeText) pbLines.push('Shape: ' + pb.shapeText);
  ...
  if (pb.spillover) pbLines.push('Spillover: ' + pb.spillover);
  ...
  const feat = (obj, title) => {
    if (obj && obj.included) sections.push({ title, price: Number(obj.price) || 0, lines: obj.details ? ['Size and Details: ' + obj.details] : [] });
  };
  feat(s.waterFeature, 'Water Feature');
  feat(s.coldPlunge, 'Cold Plunge');
