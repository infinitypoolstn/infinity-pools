...
function buildSelections(client) {
  ...
  if (s.hotTub.included) {
    const ht = [s.hotTub.details, s.hotTub.jets ? s.hotTub.jets + ' jets' : '', s.hotTub.ledLights ? s.hotTub.ledLights + ' LED lights' : ''].filter(Boolean).join(' — ');
    rows.push(['Hot Tub', ht || 'Included']);
  }
