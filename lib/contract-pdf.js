...
function buildSelections(client) {
  ...
  const rows = [];
  const pb = s.poolBase || {};
  rows.push(['Pool Shape', cap(pb.shape) + (pb.shape === 'freeform' && pb.freeform ? ' — ' + pb.freeform : '')]);
  if (pb.size) rows.push(['Size', pb.size]);
  if (pb.depth) rows.push(['Depth', pb.depth]);
  if (pb.shapeText) rows.push(['Shape', pb.shapeText]);
  if (pb.jets) rows.push(['Jets', pb.jets]);
  if (pb.ledLights) rows.push(['Hayward Colorlogic 320 LED Lights', pb.ledLights]);
  if (pb.sunShelf && pb.sunShelf.included) rows.push(['Sun Shelf', pb.sunShelf.details || 'Included']);
  if (pb.spillover) rows.push(['Spillover', pb.spillover]);
  if (pb.ledgeSeating && pb.ledgeSeating.included) rows.push(['Ledge / Seating', pb.ledgeSeating.details || 'Included']);
  const spa = s.spaBase || {};
  if (spa.included) {
    const det = [spa.size, spa.jets ? spa.jets + ' jets' : '', spa.ledLights ? spa.ledLights + ' LED lights' : ''].filter(Boolean).join(' — ');
    rows.push(['Spa', det || 'Included']);
  }
  if (s.waterFeature && s.waterFeature.included) rows.push(['Water Feature', s.waterFeature.details || 'Included']);
  if (s.coldPlunge && s.coldPlunge.included) rows.push(['Cold Plunge', s.coldPlunge.details || 'Included']);
