...
function buildOverviewText(client) {
  ...
  const parts = [];
  parts.push(`The project scope includes pre-construction design coordination followed by the construction of a custom ${s.sizeDetails ? s.sizeDetails + ' ' : ''}${cap(s.shape)} in-ground pool${s.hotTub.included ? ' with an attached spa' : ''}.`);
  const feats = [];
...
async function generate(client, { uploadsDir }) {
  ...
  // ---------------- Your Selections ----------------
  doc.addPage();
  header(doc, 'Your Selections');
  doc.font('Times-Roman').fontSize(10).fillColor(MID)
    .text('A summary of the selections and configuration for your custom pool, as captured in this proposal.', 50, 112, { width: doc.page.width - 100, lineGap: 3 });
  let selY = 148;
  for (const [label, value] of buildSelections(client)) {
    if (selY > doc.page.height - 90) { doc.addPage(); header(doc, 'Your Selections (cont.)'); selY = 120; }
    doc.font('Times-Bold').fontSize(11).fillColor(BLUE).text(label + ':', 56, selY, { width: 185 });
    doc.font('Times-Roman').fontSize(11).fillColor(DARK).text(String(value || '—'), 245, selY, { width: doc.page.width - 295 });
    selY = Math.max(selY + 24, doc.y + 12);
    doc.moveTo(50, selY - 8).lineTo(doc.page.width - 50, selY - 8).lineWidth(0.5).strokeColor('#e2edf6').stroke();
  }
