...
// Section header rendered as a medium-blue banner with white text.
function sectionTitle(doc, text) {
  if (doc.y > doc.page.height - 140) doc.addPage();
  doc.moveDown(0.6);
  const by = doc.y, h = 24;
  doc.rect(50, by, doc.page.width - 100, h).fill(MEDBLUE);
...
function buildSpecSections(client) {
  ...
  // Spa Base, Cold Plunge, and Fire Feature show the section header + price only
  // (no Size and Details lines); Water Feature keeps its detail line.
  const spa = s.spaBase || {};
  if (spa.included) sections.push({ title: 'Spa Base', price: Number(spa.price) || 0, lines: [] });
  const feat = (obj, title, showDetails) => {
    if (obj && obj.included) sections.push({ title, price: Number(obj.price) || 0, lines: showDetails && obj.details ? ['Size and Details: ' + obj.details] : [] });
  };
  feat(s.waterFeature, 'Water Feature', true);
  feat(s.coldPlunge, 'Cold Plunge', false);
  feat(s.fireFeature, 'Fire Feature', false);
...
async function generate(client, { uploadsDir }) {
  ...
  for (const sec of buildSpecSections(client)) {
    sectionTitle(doc, sec.title + '  —  ' + money(sec.price));
  ...
  doc.fillColor('#ffffff').font('Times-Bold').fontSize(14).text('Total Price Quote', 60, tqY + 10, { lineBreak: false });
  doc.font('Times-Bold').fontSize(14).fillColor('#ffffff').text(money(total), 50, tqY + 10, { width: doc.page.width - 110, align: 'right' });
  doc.fillColor(DARK);
  doc.y = tqY + 34 + 12;
  ...
  doc.rect(x0, y, tableW, rowH).fill(BLUE);
  doc.fillColor('#ffffff').font('Times-Bold').fontSize(14);
