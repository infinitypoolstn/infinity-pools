...
// Whole dollars only on the contract — no cents.
const money = n => '$' + Math.round(Number(n) || 0).toLocaleString('en-US', { minimumFractionDigits: 0, maximumFractionDigits: 0 });
...
// Items may be strings or { text, indent } — indented sub-lines get a dash
// marker and extra left inset.
...
// Measure (without rendering) how tall a run of bullets will be.
...
// Height a sectionTitle banner + its bullets will consume.
function sectionHeight(doc, items) {
  doc.font(BODY).fontSize(10);
  return doc.currentLineHeight() * 0.3 + 22 + 5 + bulletsHeight(doc, items);
}

// If the next block (given height) would spill off this page but fits on a fresh
// one, start a new page first — so the whole section stays together.
...
// Project overview paragraph, shown below the Scope of Work. Uses the client's
// (editable) text when set, otherwise the shared default.
function buildOverviewText(client) {
  const custom = client && client.projectOverview;
  return (custom && custom.trim()) ? custom : store.DEFAULT_PROJECT_OVERVIEW;
}
...
// allDetails = true also prints the free-text "Additional Details" the contract
// trims off some sections — the Plans Spec Sheet needs every note to draft from.
function buildSpecSections(client, { allDetails = false } = {}) {
  ...
  if (s.equipmentPad) pbLines.push('Equipment Pad Location: ' + s.equipmentPad);
  // Additional Details: label on its own line, then each line the user typed in
  // the textbox as its own line beneath it (blank lines dropped).
  const pushDetails = (lines, text) => {
    if (!(text || '').trim()) return;
    lines.push({ text: 'Additional Details:', bold: true });
    for (const ln of String(text).split(/\r?\n/)) if (ln.trim()) lines.push(ln.trim());
  };
  pushDetails(pbLines, pb.details);
  sections.push({ title: 'Pool Base', price: Number(pb.price) || 0, lines: pbLines });

  // Each extra priced line item on a section becomes its own "Section — Item" row,
  // matching the labels store.specsToFinance emits.
  const pushSubItems = (obj, title) => {
    for (const it of (obj.items || [])) {
      if (!(it.label || '').trim()) continue;
      sections.push({ title: title + ' — ' + it.label, price: Number(it.price) || 0, lines: it.value ? ['Details: ' + it.value] : [] });
    }
  };
  pushSubItems(pb, 'Pool Base');
  // Spa Base keeps Size / Jets / LED; on the contract its free-text additional
  // details are trimmed, but the spec sheet (allDetails) prints them.
  const spa = s.spaBase;
  if (spa && spa.included) {
    const lines = [];
    ...
    if (allDetails) pushDetails(lines, spa.details);
    sections.push({ title: 'Spa Base', price: Number(spa.price) || 0, lines });
    pushSubItems(spa, 'Spa Base');
  }
  // Fire Lounge follows Spa Base: Size only (no Jets / LED), no free-text details.
  const fl = s.fireLounge;
  if (fl && fl.included) {
    sections.push({ title: 'Fire Lounge', price: Number(fl.price) || 0, lines: fl.size ? ['Size: ' + fl.size] : [] });
    pushSubItems(fl, 'Fire Lounge');
  }
  ...
  if (cp && cp.included) {
    ...
    if (cp.additionalDetails) lines.push('Additional Details: ' + cp.additionalDetails);
    sections.push({ title: 'Cold Plunge', price: Number(cp.price) || 0, lines });
    pushSubItems(cp, 'Cold Plunge');
  }
...
async function generate(client, { uploadsDir, forSigning = false } = {}) {
  ...
  // Title block — address + quoted price + proposal line ABOVE the INFINITY POOLS
  // wordmark, left-aligned (dark on white).
  const ty = imgBottom + 42;
  doc.fillColor(DARK).font(DISPLAY).fontSize(28)
    .text(client.address.toUpperCase(), M, ty, { width: PW - 2 * M });
  // Quoted price beneath the address — no label, brand blue to set it off.
  doc.fillColor(BLUE).font(DISPLAY).fontSize(22)
    .text(money(total), M, doc.y + 4, { width: PW - 2 * M });
  ...
  // ---------------- Pool Renderings gallery ----------------
  // Renderings are grouped by design option: Option A first, then Option B on
  // its own page. A project with only one option gets a plain "Pool Renderings"
  // heading, no A/B labels.
  const renderings = client.files.filter(f => f.category === 'Pool Renderings');
  const byOpt = { A: renderings.filter(f => f.renderOption !== 'B'), B: renderings.filter(f => f.renderOption === 'B') };
  const twoOptions = byOpt.A.length > 0 && byOpt.B.length > 0;
  for (const opt of ['A', 'B']) {
    const group = byOpt[opt];
    if (!group.length) continue;
    const title = twoOptions ? `Pool Renderings — Option ${opt}` : 'Pool Renderings';
    doc.addPage();
    header(doc, title);
  ...
  // Preserve indent while cleaning obsolete text; items may be strings or {text,indent}.
  ...
  for (const sec of client.scope) {
    ...
    keepTogether(doc, sectionHeight(doc, items)); // never split a section across the page break
    sectionTitle(doc, sec.title + (drawByKey[sec.key] !== undefined && drawByKey[sec.key] > 0 ? `  —  ${drawByKey[sec.key]}% Draw` : ''));
    bullets(doc, items);
    doc.moveDown(0.35); // space each scope section out
  }
  doc.moveDown(0.4); // extra space after Landscaping, before the pool sections
  ...
  const overviewText = buildOverviewText(client);
  doc.font(BODY).fontSize(10);
  const overviewH = doc.currentLineHeight() * 0.3 + 22 + 5 + doc.heightOfString(overviewText, { width: doc.page.width - 112, lineGap: 1 });
  keepTogether(doc, overviewH); // don't split the overview paragraph across pages
  sectionTitle(doc, 'Project Overview');
  doc.font(BODY).fontSize(10).fillColor(DARK)
    .text(overviewText, 56, doc.y, { width: doc.page.width - 112, lineGap: 1 });
  doc.moveDown(0.3);
  if (doc.y > doc.page.height - 58) doc.addPage();
  const tqY = doc.y;
  ...
  const discs = (client.disclosures && client.disclosures.length) ? client.disclosures : store.data.settings.disclosures;
  discs.forEach((d, i) => {
  ...
  // Flow directly below the last disclosure (Termination Clause). keepTogether
  // only starts a new page if the whole signature block won't fit here.
  const ackPara = 'By signing below, Client acknowledges having read and understood all disclosures, exclusions, and site conditions set forth in this document, and agrees that these terms, together with the Scope of Work and Budget & Timeline, are incorporated into and made part of this signed project proposal. ' +
    'Any work, conditions, or materials not specifically included in this signed contract will be handled through a written change order prior to performance.';
  doc.font(BODY).fontSize(10);
  const ackH = 45 + doc.heightOfString(ackPara, { width: doc.page.width - 100, lineGap: 4 }) + 150; // paragraph + total + two signature lines + footer
  keepTogether(doc, ackH);
  doc.moveDown(0.6);
  sectionTitle(doc, 'Client Acknowledgment');
  doc.font(BODY).fontSize(10).fillColor(DARK).text(ackPara, 50, doc.y, { width: doc.page.width - 100, lineGap: 4 });
  doc.moveDown(0.8);
  doc.font(BOLD).fontSize(11).fillColor(BLUE).text('Contract total: ' + money(total), 50, doc.y);
  doc.moveDown(1.8);
...
module.exports = { generate, buildSpecSections };
