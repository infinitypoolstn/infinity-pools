...
async function generate({ outDir } = {}) {
  ...
  // A "check to include" toggle on the left with a Price ($) field on the right.
  function includePriceRow(label, incName, priceName) {
    need(BOX_H + ROW_GAP + 4);
    const y = st.y;
    checkbox('Include ' + label, M, y + 4, incName);
    const pw = 150;
    const px = M + contentW - pw;
    doc.font(BODY).fontSize(8.5).fillColor(MID).text('Price ($)', px, y - 8, { lineBreak: false });
    doc.formText(uniq(priceName), px, y, pw, BOX_H, { borderColor: FIELD_BORDER, backgroundColor: FIELD_BG, fontSize: 11, align: 'left' });
    st.y = y + BOX_H + ROW_GAP;
  }
  ...
  fieldRow([{ label: 'Owner / Builder', name: 'owner_builder' }]);
  fieldRow([{ label: 'Property Address', name: 'address' }]);
  fieldRow([
    { label: 'Sales Rep', name: 'sales_rep' },
  ...
  banner('Pool Base');
  need(LABEL_H + BOX_H + ROW_GAP);
  fieldAt('Price ($)', M, st.y, 200, 'pb_price');
  st.y += LABEL_H + BOX_H + ROW_GAP;
  ...
  fieldRow([{ label: 'Size', name: 'pb_size' }]);
  fieldRow([{ label: 'Depth', name: 'pb_depth' }]);
  fieldRow([
  ...
  banner('Spa Base');
  includePriceRow('Spa Base', 'spa_inc', 'spa_price');
  fieldRow([
  ...
  includePriceRow('Water Feature', 'wf_inc', 'wf_price');
  fieldRow([{ label: 'Size and Details', name: 'wf_det' }]);

  banner('Cold Plunge');
  includePriceRow('Cold Plunge', 'cp_inc', 'cp_price');
  fieldRow([{ label: 'Size and Details', name: 'cp_det' }]);

  banner('Fire Feature');
  includePriceRow('Fire Feature', 'ff_inc', 'ff_price');
  fieldRow([{ label: 'Size and Details', name: 'ff_det' }]);

  banner('Add-Ons');
  for (let i = 1; i <= 5; i++) {
    fieldRow([
      { label: `Add-on ${i}`, name: `ao_label_${i}`, flex: 1.2 },
      { label: 'Details', name: `ao_value_${i}`, flex: 1.6 },
      { label: 'Price ($)', name: `ao_price_${i}`, flex: 0.7 },
    ]);
  }

  banner('Additional Notes');
  need(150);
  doc.formText(uniq('notes'), M, st.y, contentW, 150, {
    borderColor: FIELD_BORDER, backgroundColor: FIELD_BG, fontSize: 11, align: 'left', multiline: true,
  });
  st.y += 150;
