...
async function generate({ outDir } = {}) {
  ...
  // Ruled lines to write on. Each line is also a transparent (borderless)
  // fillable field, so the form works both printed and on-screen.
  function notesLines(count) {
    const LINE_GAP = 26;
  ...
  need(40 + 26 * 4); // keep the heading with at least the first few note lines
