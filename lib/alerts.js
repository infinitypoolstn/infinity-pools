...
async function sendPaymentRequest(client, phase) {
  ...
  // Bill this draw as its own QuickBooks progress invoice (once) for the exact draw
  // amount, unless a link was pasted manually. Failures are logged, not fatal.
  if (!phase.paymentLink && !phase.qbInvoiceId && quickbooks.connected()) {
    try { await quickbooks.createDrawInvoice(client, phase, amount); }
    catch (e) { store.addError('POST', `draw-invoice/${client.id}/${phase.key}`, e.message, e.stack); }
  }
  const link = phase.paymentLink || phase.payLink || '';
